...
public enum SigactionDelayer_Unsig {
	...
	**Example**: If you register a dispatch source signal for the signal 15 but
	...
	public static func registerDelayedSigaction(_ signal: Signal, originalHandlerAction: @escaping DelayedSigactionHandler) throws -> DelayedSigaction {
		return try signalProcessingQueue.sync{
			try registerDelayedSigactionOnQueue(signal, handler: originalHandlerAction)
		}
	}
	...
	private static func processSignalsOnQueue(signal: Signal, count: UInt) {
		...
		for _ in 0..<count {
			...
			for (_, originalHandlerAction) in unsigactionedSignal.handlers {
				group.enter()
