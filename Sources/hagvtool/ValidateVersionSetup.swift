...
struct ValidateVersionSetup : ParsableCommand {
	...
	func run() throws {
		...
		try xcodeproj.managedObjectContext.performAndWait{
			...
			let allCombinedBuildSettings = try CombinedBuildSettings.allCombinedBuildSettingsForTargets(of: xcodeproj.pbxproj.rootObject, xcodeprojURL: xcodeprojURL, defaultBuildSettings: defaultBuildSettings)
