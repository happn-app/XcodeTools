...
if let envName = envName {
	...
	var envAndPwd = EnvAndPwd(pwd: FileManager.default.currentDirectoryPath)
	...
	while let curVarValC = curEnv.pointee {
		...
		envAndPwd.env[split[0]] = split[1] /* Same, if we get the same var twice, environ is invalid so we override without worrying. */
	}
	print(try String(data: encoder.encode(envAndPwd), encoding: .utf8)!)
...
//		envAndPwd.env[varBase64] = valBase64
//	}
//	print(try encoder.encode(envAndPwd).base64EncodedString())
}
